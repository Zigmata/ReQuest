...
error-invalid-image-url =
    Одна або кілька URL-адрес зображень недійсні. Discord вимагає повне посилання, що починається з `http://` або `https://` і веде безпосередньо до зображення (наприклад, `https://example.com/banner.png`).

    Будь ласка, відредагуйте квест і вкажіть дійсні URL-адреси зображень, або залиште поля порожніми.
error-invalid-image-url-field = URL-адреса поля { $fieldName } недійсна. Вкажіть повне посилання, що починається з `http://` або `https://`, або залиште порожнім.
error-field-thumbnail = мініатюра
error-field-large-image = велике зображення
