...
config-modal-title-clear-shop = 確認清空商店
...
config-modal-label-restock-increment = 補貨數量（每週期）
config-modal-placeholder-restock-increment = 每週期補貨數量（預設：1）
...
config-wizard-intro =
    ...
    此精靈將協助您確認伺服器是否已正確設定以使用 ReQuest 的功能。 它將掃描您目前的設定並提供任何需要調整的建議。

    使用下方的「啟動掃描」按鈕開始驗證流程。掃描完成後， 您將收到伺服器設定的詳細報告以及建議的變更。
...
config-label-restock-incremental = 遞增（每物品數量）
...
config-label-stock-increment = 補貨：+{ $increment }/週期
