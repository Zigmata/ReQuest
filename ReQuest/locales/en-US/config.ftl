...
config-btn-clear-shop = Clear Shop
...
config-modal-title-clear-shop = Confirm Clear Shop
...
config-modal-label-clear-shop = Clear all items from this shop?
...
config-modal-label-restock-increment = Restock Increment (per cycle)
config-modal-placeholder-restock-increment = Amount added per restock cycle (default: 1)
...
config-modal-restock-schedule-label = Schedule
config-modal-restock-schedule-none = None (Disabled)
config-modal-restock-schedule-hourly = Hourly
config-modal-restock-schedule-daily = Daily
config-modal-restock-schedule-weekly = Weekly
...
config-modal-restock-day-label = Day of Week (Weekly only)
config-modal-restock-mode-label = Restock Mode
config-modal-restock-mode-full = Full (reset to max)
config-modal-restock-mode-incremental = Incremental (add amount)
...
config-btn-example-json = Example JSON
config-desc-example-json =
    {"**"}Example JSON{"**"}
    Download an example JSON file showing the expected format.
config-msg-example-json = Here is an example JSON file showing the expected format.
...
config-label-restock-incremental = Incremental (per-item amounts)
...
config-server-language-help =
    This setting allows you to specify the default language for ReQuest's {"**"}public{"**"} responses and messages in this server. Public responses include:
    - Quest and Player Board posts
    - Quest Summary and Log Channel messages
    - Shop restocking
    - Player item consumption

    This setting only affects static text generated by the bot, and does not translate dynamic content such as user inputted item names or quest descriptions.

    Personal responses and menus are not affected by this setting.
