...
from ReQuest.utilities.constants import (
    CharacterFields, ShopFields, CommonFields, CartFields, DatabaseCollections, DiscordCharacterLimits
)
...
class ShopItemButton(Button):
    def __init__(self, item, cost_string='Free', stock_info=None, locale=None):
        ...
        if is_out_of_stock:
            label = t(locale, 'shop-btn-out-of-stock')[:DiscordCharacterLimits.BUTTON_LABEL]
            style = ButtonStyle.secondary
            disabled = True
        elif len(costs) > 1:
            label = t(locale, 'shop-btn-view-options')[:DiscordCharacterLimits.BUTTON_LABEL]
            style = ButtonStyle.success
            disabled = False
        else:
            label = t(locale, 'shop-btn-add-to-cart', cost=cost_string)[:DiscordCharacterLimits.BUTTON_LABEL]
            style = ButtonStyle.success
            disabled = False
...
class ViewCartButton(Button):
    def __init__(self, calling_view):
        ...
        super().__init__(
            label=t(locale, 'shop-btn-view-cart')[:DiscordCharacterLimits.BUTTON_LABEL],
            style=ButtonStyle.success,
...
class CartBackButton(Button):
    def __init__(self, target_view):
        ...
        super().__init__(
            label=t(locale, 'shop-btn-back-to-shop')[:DiscordCharacterLimits.BUTTON_LABEL],
            style=ButtonStyle.secondary,
...
class CartClearButton(Button):
    def __init__(self, calling_view):
        locale = getattr(calling_view, 'locale', DEFAULT_LOCALE)
        super().__init__(
            label=t(locale, 'shop-btn-clear-cart')[:DiscordCharacterLimits.BUTTON_LABEL],
...
class CartCheckoutButton(Button):
    def __init__(self, calling_view):
        locale = getattr(calling_view, 'locale', DEFAULT_LOCALE)
        super().__init__(
            label=t(locale, 'shop-btn-checkout')[:DiscordCharacterLimits.BUTTON_LABEL],
            style=ButtonStyle.success,
...
class EditCartItemButton(Button):
    def __init__(self, item_key, quantity, locale=None):
        locale = locale or DEFAULT_LOCALE
        super().__init__(
            label=t(locale, 'shop-btn-edit-quantity')[:DiscordCharacterLimits.BUTTON_LABEL],
            style=ButtonStyle.secondary,
