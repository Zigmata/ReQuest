...
from ReQuest.utilities.constants import QuestFields, DiscordCharacterLimits
...
class PartyMemberSelect(Select):
    def __init__(self, calling_view, disabled_components=None):
        ...
        super().__init__(
            placeholder=t(locale, 'gm-select-placeholder-party-member')[:DiscordCharacterLimits.SELECT_PLACEHOLDER],
            options=[],
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            confirm_modal = common_modals.ConfirmModal(
                title=t(locale, 'gm-modal-title-remove-from-quest')[:DiscordCharacterLimits.MODAL_TITLE],
                prompt_label=t(locale, 'gm-modal-label-remove-from-quest')[:DiscordCharacterLimits.LABEL_LABEL],
